...
layui.define(["element", "jquery", "layer"], function (exports) {
        ...
        $('body').on('click', '[data-one-page]', function () {
            ...
            // 隐藏底图
            console.log($(this).attr('data-one-page'));
            // 如果是首页
           
            // 如果是一半页
            if(($(this).attr('data-one-page').indexOf("rwgl.html")!=-1)||($(this).attr('data-one-page').indexOf("lsrw.html")!=-1)){
            	$(".layuimini-content-page").css("width","40%");
            	$(".layuimini-content-page").show();
            	$(".mainMap").css("width","59%");
            	$(".mainMap").show();
            	
            }else if($(this).attr('data-one-page').indexOf("fxcg.html")!=-1){
            	$(".layuimini-content-page").css("width","30%");
            	$(".layuimini-content-page").show();
            	$(".mainMap").css("width","69%");
            	$(".mainMap").show();
            }else{
            	$(".layuimini-content-page").css("width","100%");
            	$(".layuimini-content-page").show();
            	$(".mainMap").hide();
            	if(($(this).attr('data-one-page').indexOf("kongbai.html")!=-1)||($(this).attr('data-one-page').indexOf("dashuju.html")!=-1)){
	            	$(".mainMap").css("width","100%");
	            	$(".mainMap").show();
	            	if(($(this).attr('data-one-page').indexOf("dashuju.html")!=-1)){
	            		var layerHeight = $(window).height()-60+"px";
	            		var layerWidth = $(window).width()+"px";
	            		layer.open({
	            		  // title : '大数据分析',
	            		  title: false,
	            		  // closeBtn: 0,
						  ...
						  content: ['/wrj/page/dashuju2.html','no'] //这里content是一个URL，如果你不想让iframe出现滚动条，你还可以content: ['http://sentsin.com', 'no']
							,end: function(index, layero){ 
  								$('[data-one-page="page/kongbai.html"]').click()
							}    
	            		});
	            	}
	            }else{
