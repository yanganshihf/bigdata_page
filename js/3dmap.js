// 初始化图表
var myChart, myChartXB;

// JSON文件(地图数据)路径
var uploadedDataURL = "json/50.json";
	...
	var option = {
		...
		series: [{
			...
			label: { // 标签的相关设置
				...
				//formatter:,               // 标签内容格式器
				textStyle: { // 标签的字体样式
					color: 'rgba(255,255,0)', // 地图初始化区域字体颜色
					margin : 60,
					fontSize: 10, // 字体大小
					opacity: 1, // 字体透明度
			...
			viewControl: { // 用于鼠标的旋转，缩放等视角控制。
				...
				center: [0, -5, 0], // 视角中心点，旋转也会围绕这个中心点旋转，默认为[0,0,0]。
		...
		$.get('json/50.json', function(xb) {
			...
			for (i = 0; i < features.length; i++) {
				if (features[i].properties.name == clickName) {
					...
					$("#mymap2").show();
					$("#returnbtn").show();
					myChartXB = echarts.init(document.getElementById('mymap2'));
					switch (clickName) {
						case "兴安林场":
						...
						default:
							getValues();
							setTimeout(function() {
								initdata(data_random0, data_random1, data_random2, data_random3, data_random40, data_random41,
									data_random5);
							}, 500)
							break;
					}
					...
					var optionXB = {
						...
						series: [{
							type: 'map3D', // 系列类型
							name: 'map3D', // 系列名称
							map: 'xbclick', // 地图类型。echarts-gl 中使用的地图类型同 geo 组件相同(ECharts 中提供了两种格式的地图数据，一种是可以直接 script 标签引入的 js 文件，引入后会自动注册地图名字和数据。还有一种是 JSON 文件，需要通过 AJAX 异步加载后手动注册。)
							regionHeight: 5, //地图厚度
							label: { // 标签的相关设置
								show: true, // (地图上的城市名称)是否显示标签 [ default: false ]
								distance: 5,            // 标签距离图形的距离，在三维的散点图中这个距离是屏幕空间的像素值，其它图中这个距离是相对的三维距离
							...
							viewControl: { // 用于鼠标的旋转，缩放等视角控制。
								...
								distance: 195, // [ default: 100 ] 默认视角距离主体的距离，对于 grid3D 和 geo3D 等其它组件来说是距离中心原点的距离,对于 globe 来说是距离地球表面的距离。在 projection 为'perspective'的时候有效。
								minDistance: 40, // [ default: 40 ] 视角通过鼠标控制能拉近到主体的最小距离。在 projection 为'perspective'的时候有效。
								maxDistance: 400, // [ default: 400 ] 视角通过鼠标控制能拉远到主体的最大距离。在 projection 为'perspective'的时候有效。
